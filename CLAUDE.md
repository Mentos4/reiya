...
- `COLS = [(label, width), ...]` widths are derived from `cfg['dashboard_width']` (default 40, user-settable via Option 6 → 4 "Set Dashboard Table Width") — NEVER probed via `tput`/`stty`/`shutil.get_terminal_size()`. Terminal-size detection reported the wrong width around screen rotation (`set_landscape_orientation()` needs root/`su` and silently no-ops otherwise, so the terminal can keep rotating under the script) and varied by device, causing wrap glitches (a line printed exactly `cols`-wide defers its wrap, sticking the next print's first character onto the same row). If the layout is broken for a user, the fix is telling them to run `stty size` and set a `dashboard_width` a few columns narrower than the reported column count — not reintroducing size detection.
